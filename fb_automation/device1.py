...
# FETCH THE ACCOUNTS:1 OBJECTS ARRAY
a1 = {"accounts1.group": "1"}
# GLOBAL COUNTER
ctr = 0
# TO LOCK THE THREAD
lock = threading.Lock()
    ...
    if ctr < 3:
        for key in fnc(accnt_arr):
            ...
            if allowUse(account_timestamp, time_now, accnt_url, url):
                worker1_id = str(uuid.uuid4())[:8]
                print("Worker id: ", worker1_id, " is working...")
                print(password, " next...")
                ...
                # UPDATE URL
                newURL = collection.update_one({"accounts1.email": email},
                                               {"$set": {"accounts1.$.url": url}})
                # UPDATE THE TIME
                newTimestamp = collection.update_one({"accounts1.email": email},
                                                     {"$set": {"accounts1.$.timestamp": time_now}})
                cp1.login(url, email, password)
                filename = str(uuid.uuid4())[:8]
                cp1.comment(comment, filename)
                img_url = "http://127.0.0.1:5000/static/img/" + filename + ".png"
                webhook_url_extension = "/static/img/" + filename + ".png"
                discord_webhook.webhook(img_url, worker1_id, webhook_url_extension)
        ...
        print("All accounts were exhausted from device 1...returning to the first account")
        ctr = 0


def main(url, comment, time_now):
    worker("accounts1", arr, a1, url, comment, time_now)
